export const ease = {
  in: {
    back: x => {
      const c1 = 1.70158;
      const c3 = c1 + 1;
      return c3 * x * x * x - c1 * x * x;
    },
    bounce: x => 1 - ease.out.bounce(1 - x),
    circ: x => 1 - Math.sqrt(1 - Math.pow(x, 2)),
    cubic: x => x * x * x,
    elastic: x => {
      const c4 = (2 * Math.PI) / 3;
      return x === 0 ? 0 : x === 1 ? 1 : -Math.pow(2, 10 * x - 10) * Math.sin((x * 10 - 10.75) * c4);
    },
    expo: x => (x === 0 ? 0 : Math.pow(2, 10 * x - 10)),
    sine: x => 1 - Math.cos((x * Math.PI) / 2),
    quad: x => x * x,
    quart: x => x * x * x * x,
    quint: x => x * x * x * x * x,
  },

  out: {
    back: x => {
      const c1 = 1.70158;
      const c3 = c1 + 1;
      return 1 + c3 * Math.pow(x - 1, 3) + c1 * Math.pow(x - 1, 2);
    },
    bounce: x => {
      const n1 = 7.5625;
      const d1 = 2.75;
      if (x < 1 / d1) {
        return n1 * x * x;
      } else if (x < 2 / d1) {
        return n1 * (x -= 1.5 / d1) * x + 0.75;
      } else if (x < 2.5 / d1) {
        return n1 * (x -= 2.25 / d1) * x + 0.9375;
      } else {
        return n1 * (x -= 2.625 / d1) * x + 0.984375;
      }
    },
    circ: x => Math.sqrt(1 - Math.pow(x - 1, 2)),
    cubic: x => 1 - Math.pow(1 - x, 3),
    elastic: x => {
      const c4 = (2 * Math.PI) / 3;
      return x === 0 ? 0 : x === 1 ? 1 : Math.pow(2, -10 * x) * Math.sin((x * 10 - 0.75) * c4) + 1;
    },
    expo: x => (x === 1 ? 1 : 1 - Math.pow(2, -10 * x)),
    sine: x => Math.sin((x * Math.PI) / 2),
    quad: x => 1 - (1 - x) * (1 - x),
    quart: x => 1 - Math.pow(1 - x, 4),
    quint: x => 1 - Math.pow(1 - x, 5),
  },

  inOut: {
    back: x => {
      const c1 = 1.70158;
      const c2 = c1 * 1.525;
      return x < 0.5
        ? (Math.pow(2 * x, 2) * ((c2 + 1) * 2 * x - c2)) / 2
        : (Math.pow(2 * x - 2, 2) * ((c2 + 1) * (x * 2 - 2) + c2) + 2) / 2;
    },
    bounce: x => (x < 0.5 ? (1 - ease.out.bounce(1 - 2 * x)) / 2 : (1 + ease.out.bounce(2 * x - 1)) / 2),
    circ: x => (x < 0.5 ? (1 - Math.sqrt(1 - Math.pow(2 * x, 2))) / 2 : (Math.sqrt(1 - Math.pow(-2 * x + 2, 2)) + 1) / 2),
    cubic: x => (x < 0.5 ? 4 * x * x * x : 1 - Math.pow(-2 * x + 2, 3) / 2),
    elastic: x => {
      const c5 = (2 * Math.PI) / 4.5;
      return x === 0
        ? 0
        : x === 1
        ? 1
        : x < 0.5
        ? -(Math.pow(2, 20 * x - 10) * Math.sin((20 * x - 11.125) * c5)) / 2
        : (Math.pow(2, -20 * x + 10) * Math.sin((20 * x - 11.125) * c5)) / 2 + 1;
    },
    expo: x => (x === 0 ? 0 : x === 1 ? 1 : x < 0.5 ? Math.pow(2, 20 * x - 10) / 2 : (2 - Math.pow(2, -20 * x + 10)) / 2),
    sine: x => -(Math.cos(Math.PI * x) - 1) / 2,
    quad: x => (x < 0.5 ? 2 * x * x : 1 - Math.pow(-2 * x + 2, 2) / 2),
    quart: x => (x < 0.5 ? 8 * x * x * x * x : 1 - Math.pow(-2 * x + 2, 4) / 2),
    quint: x => (x < 0.5 ? 16 * x * x * x * x * x : 1 - Math.pow(-2 * x + 2, 5) / 2),
  },

  linear: x => x,

  cubicBezier: (X1, Y1, X2, Y2) => ease.custom(`M 0 0 C ${X1} ${Y1} ${X2} ${Y2} 1 1`),
  
  custom: d => {
    const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    path.setAttribute('d', d);
    const pathLength = path.getTotalLength();
    return x => {
      let start = 0;
      let end = pathLength;
      let target = (start + end) / 2;
      while (target >= start && target <= pathLength) {
        const pos = path.getPointAtLength(target);
        if (Math.abs(pos.x - x) <= (x === 1 ? 0 : 0.001)) {
          return pos.y;
        } else if (pos.x >= x) {
          end = target;
        } else {
          start = target;
        }
        target = (start + end) / 2;
      }
    };
  },
};
