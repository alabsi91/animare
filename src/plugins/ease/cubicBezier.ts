/**
 * https://github.com/gre/bezier-easing BezierEasing - use bezier curve for transition easing function by Gaëtan Renaudeau 2014 -
 * 2015 – MIT License
 */
...
const getSlope = (aT: number, aA1: number, aA2: number) => 3 * A(aA1, aA2) * aT * aT + 2 * B(aA1, aA2) * aT + C(aA1);
...
function binarySubdivide(aX: number, aA: number, aB: number, mX1: number, mX2: number) {
  ...
  } while (Math.abs(currentX) > SUBDIVISION_PRECISION && ++index < SUBDIVISION_MAX_ITERATIONS);

  return currentT;
}

function newtonRaphsonIterate(aX: number, aGuessT: number, mX1: number, mX2: number) {
  for (let index = 0; index < NEWTON_ITERATIONS; ++index) {
...
export default function cubicBezier(mX1: number, mY1: number, mX2: number, mY2: number): EaseFunction {
  ...
  for (let index = 0; index < kSplineTableSize; ++index) sampleValues[index] = calcBezier(index * kSampleStepSize, mX1, mX2);
  ...
  function getTForX(aX: number) {
    ...
    const distribution = (aX - sampleValues[currentSample]) / (sampleValues[currentSample + 1] - sampleValues[currentSample]),
      guessForT = intervalStart + distribution * kSampleStepSize,
