...
The easing decides the value at every frame, the first and the last one included, like CSS. An easing that does not return exactly `0` and `1` at the edges will not start at `from` or end at `to`.
